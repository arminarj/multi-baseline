...
class Multimodal_Datasets(Dataset):
    def __init__(self, dataset_path, data='mosei_senti', split_type='train', if_align=True):
        ...
        self.vision = torch.tensor(dataset[split_type]['vision']).double().cpu().detach()
        self.vision = self.normalize(self.vision)
        self.text = torch.tensor(dataset[split_type]['text']).double().cpu().detach()
        self.text = self.normalize(self.text) 
        self.audio = dataset[split_type]['audio']
        self.audio[self.audio == -np.inf] = 0
        self.audio = torch.tensor(self.audio).double().cpu().detach()
        self.audio = self.normalize(self.audio)
        self.labels = torch.tensor(dataset[split_type]['labels']).double().cpu().detach()
